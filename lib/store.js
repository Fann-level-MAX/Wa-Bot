...
function bind(client) {
    ...
    function updateNameToDb(contacts) {
        ...
        try {
            ...
            for (const contact of contacts) {
                ...
                if (!chats) chats = client.chats[id] = { ...contact, id }
                client.chats[id] = {
                    ...chats,
                    ...({
    ...
    client.ev.on('contacts.set', updateNameToDb)
    ...
    client.ev.on('chats.upsert', function chatsUpsertPushToDb(chatsUpsert) {
        try {
            const { id, name } = chatsUpsert
            if (!id || id === 'status@broadcast') return
            client.chats[id] = { ...(client.chats[id] || {}), ...chatsUpsert, isChats: true }
            const isGroup = id.endsWith('@g.us')
            if (isGroup) client.insertAllGroup().catch(_ => null)
        } catch (e) {
            console.error(e)
        }
    })
